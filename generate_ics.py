...
# Rotated at random on each page load. "No korfuffle." is weighted more
# heavily than the rest; the last few are deliberately daft, as an easter egg.
TAGLINES = [
    ("No korfuffle.", 5),
    ("Korfball. But organised.", 2),
    ("Never miss a throw-off.", 2),
    ("Fixtures. Sorted.", 2),
    ("Korfball, calendarised.", 2),
    ("Your fixtures. Where they belong.", 2),
    ("Subscribe. Forget. Play.", 2),
    ("Less fixture faff.", 2),
    ("Know where. Know when. Korf.", 2),
    ("One less thing to remember.", 2),
    ("Put some Korf in your Kal.", 2),
    ("All your fixtures. None of the korfuffle.", 1),
    ("Probably better than a WhatsApp message.", 1),
    ("We checked the fixture list so you don\u2019t have to.", 1),
    ("The world\u2019s second-best korfball calendar.", 1),
]

TAGLINE_JS = """<script>
(function(){
 var pool=%s, n=0, i, r;
 for(i=0;i<pool.length;i++) n+=pool[i][1];
 r=Math.random()*n;
 for(i=0;i<pool.length;i++){ r-=pool[i][1]; if(r<=0) break; }
 var el=document.getElementById('tagline');
 if(el) el.textContent=pool[i][0];
})();
</script>"""
...
(function(){
 ...
 document.querySelectorAll('.next').forEach(function(el){
  var fx=JSON.parse(el.getAttribute('data-fx')||'[]'), i;
  for(i=0;i<fx.length;i++) if(fx[i].d>=today) break;
  if(i>=fx.length){ el.textContent='Season complete'; return; }
  var f=fx[i], p=f.d.split('-'), when=(+p[2])+' '+M[+p[1]-1];
  el.innerHTML='Next: <b>'+when+(f.t?' \u00b7 '+f.t:' \u00b7 TBC')+'</b> '+
   f.o.replace(/&/g,'&amp;').replace(/</g,'&lt;')+' ('+f.h+')';
 });
})();
</script>"""
 ...
 *{box-sizing:border-box}
 ...
 .kicker{font-size:1.15rem;font-weight:800;margin:.35rem 0 .1rem}
 .tagline{color:var(--orange);font-weight:800;font-style:italic;margin:.1rem 0 0}
 .pick td{padding:.95rem .4rem}
 .pick .team{font-size:1.06rem;font-weight:800}
 .pick .next{color:var(--muted);font-size:.85rem;font-weight:400;margin-top:.15rem}
 .pick .next b{color:var(--ink);font-weight:700}
 /* fixture list */
 .fx{margin-top:.5rem}
...
<header>
  ...
  <p class=tagline id=tagline>No korfuffle.</p>
...
<p class=note><strong>Heja is the source of truth.</strong> These calendars are
generated from the club's Heja. If the two ever disagree, believe Heja — and tell
whoever looks after the fixtures so this gets fixed.</p>
<p class=warn><strong>Your calendar will not update instantly.</strong> Google
refreshes subscribed calendars roughly every <strong>8&ndash;24 hours</strong>, and
Apple every few hours. That is fine for a fixture list set weeks ahead, but for a
late change — a postponement, a venue swap on the day — <strong>check Heja, not your
calendar app</strong>.</p>
<footer>Generated by KorfKal ·
<a href="https://bromleykorfball.com/">bromleykorfball.com</a></footer>
{TAGLINE_JS % json.dumps([list(t) for t in TAGLINES])}
</html>
"""


def next_fixture_data(club, fixtures, team):
    """Compact per-fixture data so the page can show the next game client-side.

    Computed in the browser rather than baked in, so the "next fixture" stays
    correct as the season progresses without regenerating the site.
    """
    out = []
    for f in fixtures:
        if team:
        ...
        out.append({
            "d": f"{f['date']:%Y-%m-%d}",
            "t": f"{f['throw_off']:%H:%M}" if f["throw_off"] else "",
            "o": ("v " if team else "") + who,
            "h": "H" if is_home else "A",
        })
    return out
...
def build_index(club, entries, base_url):
    ...
    for filename, label, count, page, data in entries:
        ...
        rows.append(
            f'<tr><td><div class=team>{html.escape(label)}</div>'
...
<header>
  ...
  <p class=tagline id=tagline>No korfuffle.</p>
  <p class=tag>{html.escape(club)} Korfball fixtures, automatically in your calendar.</p>
</header>
<h2 class=kicker>Which team do you follow?</h2>
<p class=sub>Choose your team, subscribe, and forget about it — your calendar
